...
export const BadgePickerModal: React.FC<BadgePickerModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-4xl rounded-2xl bg-slate-900 border border-slate-700/80 shadow-2xl p-4 sm:p-6 overflow-hidden text-slate-100 flex flex-col max-h-[90vh]"
      ...
      >
        ...
        {/* Top Avatar Source Selector */}
        <div className="grid grid-cols-3 gap-2 mt-3 p-1 rounded-xl bg-slate-950 border border-slate-800 shrink-0">
          <button
          ...
          >
            ...
            <span className="truncate">Mastery Feat</span>
          </button>
        </div>

        {/* Catalog Navigation Tabs */}
        <div className="flex items-center gap-2 mt-3 border-b border-slate-800 pb-2 shrink-0">
          <button
        ...
        {/* Body Content - Tab 1: 1000 Level Badges */}
        {activeCatalogTab === 'level' && (
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 mt-3 overflow-hidden min-h-0">
            {/* Left Column: Tier Tabs */}
              ...
              {TIERS.map((tier) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!isTierUnlocked && <Lock className="w-3.5 h-3.5 text-slate-600 shrink-0" />}
                  </button>
                );
              })}
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center p-2 rounded-xl border transition-all relative ${
                        isItemPreview
                          ? 'bg-violet-600/20 border-violet-500 shadow-md scale-105'
                          : isUnlocked
            ...
            {/* Right Column: Large Preview & Equip Action */}
            <div className="flex flex-col items-center justify-between p-4 rounded-2xl bg-slate-950/60 border border-slate-800 text-center">
              ...
              <div className="w-full pt-4 border-t border-slate-800/80 mt-2">
                {isPreviewLevelUnlocked ? (
                  ...
                  <div className="flex items-center justify-center gap-1.5 text-xs text-amber-400 font-semibold p-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
                    ...
                    <span>Reach Level {previewLevel} to Unlock</span>
                  </div>
                )}
              </div>
            </div>
        ...
        {/* Body Content - Tab 2: Mastery & Combined Feats */}
        {activeCatalogTab === 'mastery' && (
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 mt-3 overflow-hidden min-h-0">
            {/* Left Column: Category Filter Chips */}
            <div className="space-y-1 overflow-y-auto pr-1 custom-scrollbar">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 px-2">
                Feat Categories
              </p>
              {[
                ...
                { id: 'squares', label: 'Squares Mastery (1–100²)', count: 4 },
                { id: 'cubes', label: 'Cubes Mastery (1–30³)', count: 3 },
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center p-2.5 rounded-xl border text-center transition-all relative ${
                        isItemPreview
                          ? 'bg-violet-600/20 border-violet-500 shadow-md scale-102'
                          : badge.isUnlocked
                          ? 'bg-slate-950/60 border-slate-800 hover:border-slate-700 hover:bg-slate-800/50'
            ...
            <div className="flex flex-col items-center justify-between p-4 rounded-2xl bg-slate-950/60 border border-slate-800 text-center">
              ...
              <div className="w-full pt-4 border-t border-slate-800/80 mt-2">
                {previewMasteryBadge.isUnlocked ? (
                  ...
                  <div className="flex items-center justify-center gap-1.5 text-xs text-amber-400 font-semibold p-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
                    <Lock className="w-3.5 h-3.5" />
                    <span>Locked — Complete Drill Milestone</span>
                  </div>
                )}
              </div>
            </div>
